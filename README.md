...
สถานะปัจจุบัน: **Phase 8 — ตัวติดตั้งดับเบิลคลิกพร้อมแจก** 🎉 build เป็น
`.exe` (Windows) / `.dmg` (macOS) ผ่าน GitHub Actions โดยฝัง backend มาให้
สตาร์ท/ดับเองอัตโนมัติ + หน้าจอครั้งแรกดาวน์โหลดโมเดลในแอป (ดูวิธีติดตั้งด้านล่าง)
วงจรงานครบ: ถอดเสียง/ตรึงบท → แก้บท → ตัดเสียง → Export (WAV + .docx/.txt)
ทุกอย่างออฟไลน์ ต้นฉบับไม่ถูกแตะ
